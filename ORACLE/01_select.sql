...
-- AS : AliaS = 별명 = 별칭
특정 테이블이나 컬럼명을 개발자가 원하는 표기로 설정
SELECT FROM ORDER BY 에서 사용

컬럼명 AS 별칭 = 문자O 띄어쓰기X 특수문자X ('_' 로 띄어쓰기 한다)
컬럼명 AS "별칭" = 문자O 띄어쓰기O 특수문자O
컬럼명 별칭 = 문자O 띄어쓰기X 특수문자X
컬럼명 "별칭" = 문자O 띄어쓰기O 특수문자O

"   " 사이 글자(대/소문자,특문,띄어쓰기) 그대로 인식
'   ' 오라클에서 문자열
