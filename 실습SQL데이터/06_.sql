...
order by
    ...
    -- coalesce = 컬럼에 빈값 존재시 빈값을 0 으로 대체해서 합계 계산
    ...
    -- 에러코드 1050 
    -- Error Code: 1050. Table 'view_status_null' already exists
    -- 삭제 또는 교체하기
    -- drop view view_status_null as ~ ;
    -- create or replace view view_status_null as ~ ;
    -- oracle 은 coalesce 대신 nvl null 값을 특정값 대체
