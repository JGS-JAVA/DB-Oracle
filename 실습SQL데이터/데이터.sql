...
-- ORDER 는 SQL 예약어 ORDER BY 와 충돌한다alter
-- 백틱 `` 로 감싸서 이름 사용 
CREATE TABLE `ORDER` (
...
-- DROP TABLE IF EXISTS REVIEW; 표 지우기
...
USE KHTSHOPPING;
select count(ORDER_ID) FROM `ORDER`where status = 'pending'; 
...
select name, price
from menu order by price desc limit 1;

select table_id, total_amount
from cafe_order where status = 'preparing';
...
-- update 문 실행, 테이블 삭제시 safe 모드 생성
-- safe 모드 켜고 1/ 끄기 0
...
-- 테이블 자체 컬럼규칙 수정
-- STATUS 안에는 AVAILABLE OCCUPIED 만 사용가능
-- enum('AVAILABLE' , 'OCCUPIED') 2개 중 1개 선택
-- CREATE TABLE ENUM = HTML의 라디오
-- create TABLE SET = HTML의 체크박스

-- alter table 수정할 표이름 MODIFY COLUMN 컬럼명 컬럼명규칙

ALTER table TABLES
MODIFY column STATUS ENUM('AVAILABLE','OCCUPIED','사용 가능','사용중'); -- 4개 중 1개 선택

update tables set status = '사용중'
...
MODIFY column STATUS ENUM('PENDING','PREPARING','COMPLETED','CANCELED','사용중','준비중','완료','대기중','취소');

-- SET FOREIGN_KEY_CHECKS = 0; -- 외래키 종료
-- DROP TABLE IF EXISTS 표이름; -- 표 지우기
-- SET FOREIGN_KEY_CHECKS = 1; -- 외래키 시작
